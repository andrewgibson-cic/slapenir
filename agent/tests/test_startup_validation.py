...
def test_container_to_proxy() -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_error(
                f"Proxy health check failed with status: {response.status_code}"
            )
            return False
...
def test_proxy_routing() -> bool:
    ...
    if "proxy:3000" not in http_proxy or "proxy:3000" not in https_proxy:
        print_error(
            f"Proxy not configured correctly: HTTP_PROXY={http_proxy}, HTTPS_PROXY={https_proxy}"
        )
        return False
...
def test_llama_server_localhost() -> bool:
    """Test llama-server connectivity via localhost (127.0.0.1)"""
    print_test("Llama Server Localhost Connectivity Test")

    llama_host = os.getenv("LLAMA_SERVER_HOST", "host.docker.internal")
    llama_port = os.getenv("LLAMA_SERVER_PORT", "8080")

    # Test via host.docker.internal
    try:
        response = requests.get(
            f"http://{llama_host}:{llama_port}/v1/models", timeout=3
        )
        if response.status_code == 200:
            print_success(f"llama-server accessible at {llama_host}:{llama_port}")
        else:
            print_warning(
                f"llama-server at {llama_host}:{llama_port} returned status {response.status_code}"
            )
    except requests.exceptions.ConnectionError:
        print_warning(f"llama-server not accessible at {llama_host}:{llama_port}")
    except Exception as e:
        print_warning(f"Error connecting to {llama_host}:{llama_port}: {e}")

    # Test via 127.0.0.1 (localhost bypass rule)
    try:
        response = requests.get(f"http://127.0.0.1:{llama_port}/v1/models", timeout=3)
        if response.status_code == 200:
            print_success(
                f"llama-server accessible at 127.0.0.1:{llama_port} (localhost bypass working)"
            )
            return True
        else:
            print_warning(
                f"llama-server at 127.0.0.1:{llama_port} returned status {response.status_code}"
            )
            return False
    except requests.exceptions.ConnectionError:
        print_warning(
            f"llama-server not accessible at 127.0.0.1:{llama_port} (localhost bypass may not be active)"
        )
        return False
    except Exception as e:
        print_error(f"Error connecting to 127.0.0.1:{llama_port}: {e}")
        return False
...
def run_all_tests() -> Tuple[int, int]:
    ...
    tests = [
        ...
        ("Llama Server Localhost", test_llama_server_localhost),
    ]
