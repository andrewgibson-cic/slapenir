...
impl AuthStrategy for AWSSigV4Strategy {
    ...
    fn dummy_patterns(&self) -> Vec<String> {
        vec!["AKIADUMMY".to_string(), "AKIA00000000DUMMY".to_string()]
    }
...
mod tests {
    ...
    fn test_extract_service_from_host() {
        assert_eq!(
            AWSSigV4Strategy::extract_service_from_host("s3.amazonaws.com"),
            "s3"
        );
        assert_eq!(
            AWSSigV4Strategy::extract_service_from_host("dynamodb.us-east-1.amazonaws.com"),
            "dynamodb"
        );
        assert_eq!(
            AWSSigV4Strategy::extract_service_from_host("lambda.eu-west-1.amazonaws.com"),
            "lambda"
        );
    }
    ...
    fn test_matches_wildcard() {
        assert!(AWSSigV4Strategy::matches_wildcard(
            "*.amazonaws.com",
            "s3.amazonaws.com"
        ));
        assert!(AWSSigV4Strategy::matches_wildcard(
    ...
    fn test_aws_strategy_detect() {
        ...
        headers.insert(
            "authorization",
            HeaderValue::from_static("AWS4 AKIADUMMY..."),
        );
