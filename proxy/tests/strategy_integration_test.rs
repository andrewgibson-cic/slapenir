...
fn test_config_loading_from_file() {
    ...
    assert!(!config.strategies.is_empty(), "Should have strategies defined");
    // Security config is always present
    assert!(config.security.block_telemetry, "Should have telemetry blocking enabled");
}

#[test]
fn test_strategy_builder_from_config() {
    // Load config and build strategies
    let config = Config::from_file("config.yaml.example")
    ...
    assert!(!strategies.is_empty(), "Should build at least some strategies");
}
...
fn test_config_has_aws_strategy() {
    ...
    let has_aws = config.strategies.iter()
...
fn test_config_has_telemetry_blocking() {
    ...
    assert!(config.security.block_telemetry, "Telemetry blocking should be enabled");
    assert!(!config.security.telemetry_domains.is_empty(), "Should have telemetry domains listed");
}
...
fn test_config_has_bearer_strategies() {
    ...
    let bearer_count = config.strategies.iter()
        .filter(|s| s.strategy_type == "bearer")
        .count();
    
    assert!(bearer_count >= 3, "Should have at least 3 bearer strategies (OpenAI, Anthropic, GitHub)");
}